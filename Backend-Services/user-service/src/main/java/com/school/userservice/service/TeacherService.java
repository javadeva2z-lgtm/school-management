...
public class TeacherService {
    ...
    public TeacherDTO createTeacher(TeacherDTO teacherDTO) {
        ...
        if (teacherRepository.findByEmployeeIdOrUsername(teacherDTO.getEmployeeId(), teacherDTO.getUsername())
                .isPresent()) {
            throw new DuplicateResourceException("Teacher", "employeeId/username",
                    teacherDTO.getEmployeeId() + "," + teacherDTO.getUsername());
        }
    ...
    public List<TeacherDTO> byLevel(TeacherClassLevel level) {
        log.info("Fetching all teachers for class level :" + level);
        List<Teacher> teachers = teacherRepository.findByLevel(level);
        return teachers.stream()
                .map(teacherConverter::entityToDTO)
                .collect(Collectors.toList());
    }
    ...
    public void importCsv(MultipartFile file) {
        try (BufferedReader fileReader = new BufferedReader(
            ...
            for (CSVRecord record : csvRecords) {
                ...
                long id = record.isMapped(Constants.IMPORT_TEACHER_COLUMN_ID)
                        && !record.get(Constants.IMPORT_TEACHER_COLUMN_ID).isBlank()
                                ? Long.parseLong(record.get(Constants.IMPORT_TEACHER_COLUMN_ID))
                                : 0;
                ...
                String email = record.isMapped(Constants.IMPORT_TEACHER_COLUMN_EMAIL)
                        ? record.get(Constants.IMPORT_TEACHER_COLUMN_EMAIL)
                        : "";
                String gender = record.isMapped(Constants.IMPORT_TEACHER_COLUMN_GENDER)
                ...
                teachersToSave.add(Teacher.builder()
                        ...
                        .joiningDate(
                                joiningDate != null && !joiningDate.isBlank() ? LocalDate.parse(joiningDate) : null)
                        .experienceYears(experienceYears != null && !experienceYears.isBlank()
