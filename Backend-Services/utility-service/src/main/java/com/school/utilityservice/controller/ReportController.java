...
public class ReportController {
    ...
    public ResponseEntity<byte[]> csv(@Valid @RequestBody ReportRequest request) throws IOException {
        return download(service.csv(request), request.getTitle() + ".csv", "text/csv");
    }

    @PostMapping(value = "/pdf", produces = MediaType.APPLICATION_PDF_VALUE)
    public ResponseEntity<byte[]> pdf(@Valid @RequestBody ReportRequest request) throws DocumentException {
        return download(service.pdf(request), request.getTitle() + ".pdf", MediaType.APPLICATION_PDF_VALUE);
    }
    ...
    private ResponseEntity<byte[]> download(byte[] content, String fileName, String mediaType) {
        return ResponseEntity.ok().contentType(MediaType.parseMediaType(mediaType))
                .header(HttpHeaders.CONTENT_DISPOSITION,
                        "attachment; filename=\"" + fileName.replaceAll("[^a-zA-Z0-9._-]", "_") + "\"")
                .body(content);
    }
}
